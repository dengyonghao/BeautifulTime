...
#define MallocIndexByte (INDEX_NUM_MAX*sizeof(int64_t))
#define MallocByte (MALLOC_SIZE*sizeof(int64_t))
...
{	
	...
	int64_t** pIndexData;	  //索引空间首地址
	int pIndexNum;        //索引空间个数

	int64_t* pDataEnd;

	void (*Append)(struct Array* A, int64_t value);
	void (*Insert)(struct Array* A, int64_t value,int pos);
	void (*Remove)(struct Array* A,int index);
	void (*Reset)(struct Array* A);
	int64_t (*GetValue)(struct Array* A,int index);

}Array;

void ArrayInit(struct Array* A);
void ArrayAppend(Array* A,int64_t value);
void ArrayInsert(Array* A,int64_t value,int pos);
void ArrayRemove(Array* A,int index);
void ArrayReset(Array* A);
int ArrayReSize(Array* A);
int64_t ArrayGetValue(Array* A,int index);
